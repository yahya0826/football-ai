...
import api, { MatchDetailResponse, MatchHighlightsResponse, H2HData, RecentMatch } from '@/lib/api';
...
export default function MatchDetailPage({ params }: PageProps) {
  ...
  useEffect(() => {
    ...
    console.log('[LiveLookup] Looking up:', m.home_team, 'vs', m.away_team, 'on', m.date);
    ...
    <div style={{ maxWidth: 800, margin: '0 auto', padding: '1.5rem 1rem' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
        <SectionCard title={`${match.home_team_cn} 近期战绩`}>
          <RecentForm recent={detail.home_recent} teamCn={match.home_team_cn} />
        </SectionCard>
        <SectionCard title={`${match.away_team_cn} 近期战绩`}>
          <RecentForm recent={detail.away_recent} teamCn={match.away_team_cn} />
        </SectionCard>
      </div>
...
function RecentForm({ recent, teamCn }: { recent: RecentMatch[] | null; teamCn: string }) {
